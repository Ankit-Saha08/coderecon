...
import type {
  Decision, DiffMode, FileEntry, FilterKey, ScanOptions, Screen, SessionFile, SourceFile,
} from '@/types';
...
export const useReconStore = create<ReconState>((set, get) => ({
  ...
  runScan: async () => {
    ...
    // Throttled progress — updating on all 3,000 files would thrash React.
    const tick = (label: string) => (done: number, total: number) => {
      if (done === total || done % 25 === 0) set({ stage: label, progress: { done, total } });
    };

    set({ isScanning: true, scanError: null, stage: 'Reading folder A…', progress: { done: 0, total: filesA.length } });
    try {
  ...
  setDecision: (id, decision) =>
    set((s) => ({ entries: s.entries.map((e) => (e.id === id ? { ...e, decision, reviewed: true } : e)) })),
  setNote: (id, note) =>
  ...
  bulkDecide: (ids, decision) => {
    ...
    set((s) => ({
      entries: s.entries.map((e) => (targets.has(e.id) ? { ...e, decision, reviewed: true } : e)),
    }));
  },
  ...
  applySession: (session) => {
    ...
    set((s) => ({
      entries: s.entries.map((e) => {
        ...
        if (!saved) return e;
        applied++;
        return { ...e, decision: saved.decision, reviewed: saved.reviewed, note: saved.note ?? e.note };
      }),
    }));
    const ids = new Set(get().entries.map((e) => e.id));
    return {
      applied,
      missing: get().entries.filter((e) => !byId.has(e.id)).length,
      extra: session.decisions.filter((d) => !ids.has(d.id)).length,
    };
  },
